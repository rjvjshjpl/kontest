...
bool Solver::getAllValues(const Query& query, std::vector< ref<ConstantExpr> > &results, unsigned maxCount) {
  auto handler = [&](const ref<Expr> &target, const ref<ConstantExpr> &value) {
    results.push_back(value);
    if (--maxCount == 0)
      return ref<Expr>();
    else
      return NeExpr::create(target, value);
  };
  return getAllValues(query, handler);
}

bool Solver::getAllValues(const Query& query, const GetAllValuesHandler &handler) {
  // Maintain invariants implementations expect.
  ref<Expr> target = query.expr;
  ...
  while (true) {
    // get a possible value
    const Query currentQuery = Query(constraints, target);
    ref<ConstantExpr> value;
    bool success = getValue(currentQuery, value);
    if (!success)
      return false;

    // report value and check if others are possible
    ref<Expr> blocker = handler(target, value);
    if (blocker.isNull())
      return false;
    bool res;
    success = mayBeTrue(Query(constraints, blocker), res);
    if (!success)
      return false;

    // if not, stop; otherwise block the value we found
    if (res) {
      constraints.addConstraint(blocker);
#ifdef DEBUG_EXPENSIVE
      // Verify that blocker actually blocks value, so that we don't loop forever
      ref<Expr> hasCurrentValue = EqExpr::create(target, value);
      mayBeTrue(Query(constraints, hasCurrentValue), res);
      if (res) {
        ExprPPrinter::printQuery(llvm::errs(), constraints, hasCurrentValue);
        assert(0 && "blocker does not block given value!");
      }
#endif   // DEBUG_EXPENSIVE
    } else
      break;
  }
