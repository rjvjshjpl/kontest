...
static void __create_new_dfile(exe_disk_file_t *dfile, unsigned size, 
  ...
  memset(dfile->contents, 0, dfile->size);    // for deterministic initial concrete value
  klee_make_symbolic(dfile->contents, dfile->size, name);

  // set up initial concrete value according to defaults
  memset(s, 0, sizeof(*s));
  ...
  /* For broken tests */
  if (!klee_is_symbolic_in_symbolic_mode(s->st_ino) &&
...
void klee_init_fds(unsigned n_files, unsigned file_length,
  ...
  if (__exe_fs.max_failures) {
    ...
    memset(__exe_fs.read_fail, 0, sizeof(*__exe_fs.read_fail));   // for deterministic initial concrete value
    memset(__exe_fs.write_fail, 0, sizeof(*__exe_fs.write_fail));
    memset(__exe_fs.close_fail, 0, sizeof(*__exe_fs.close_fail));
    memset(__exe_fs.ftruncate_fail, 0, sizeof(*__exe_fs.ftruncate_fail));
    memset(__exe_fs.getcwd_fail, 0, sizeof(*__exe_fs.getcwd_fail));
  ...
  unsigned mv = 1;
  klee_make_symbolic(&mv, sizeof(mv), "model_version");
  __exe_env.version = mv;
  __exe_env.version = klee_make_concrete(__exe_env.version);
}
