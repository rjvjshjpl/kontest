...
namespace {
  ...
  cl::opt<bool>
  WithKontestRuntime("kontest-runtime",
                     cl::desc("Link with Kontest runtime (default=on)."),
                     cl::init(true));
...
extern cl::opt<bool> OutputIStats;
...
public:
  ...
  void processTestCase(const ExecutionState  &state,
                       ...
                       unsigned testID,
                       bool suppressExitOnError = false);

  bool needsSymbolicExecution() const { return WriteKQueries || WriteCVCs || WriteSMT2s; };
...
void KleeHandler::processTestCase(const ExecutionState &state,
                                  ...
                                  const char *errorSuffix,
                                  unsigned id,
                                  bool suppressExitOnError) {
  if (errorMessage && ExitOnError && !suppressExitOnError) {
    m_interpreter->prepareForEarlyExit();
    klee_error("EXITING ON ERROR:\n%s\n", errorMessage);
  }

  if (!NoOutput) {
    // detect if this test has been executed before; each test may be executed
    // at most twice (concretely and/or symbolically)
    bool alreadyRanConcretely = (id > 0);
    if (!alreadyRanConcretely)
      id = ++m_testIndex;

    bool ranSymbolically;
    KTest *b = m_interpreter->getSymbolicSolution(state, id, ranSymbolically);

    if (!b)
      klee_warning("unable to get symbolic solution, losing test case");

    if (b) {
      // if this was the first time this test was executed, write it to a file
      if (!alreadyRanConcretely) {
        b->numArgs = m_argc;
        b->args = m_argv;
        b->symArgvs = 0;
    ...
    // if this was the first time the test was executed, output various data
    // about the concrete execution
    if (!alreadyRanConcretely) {
      if (errorMessage) {
    ...
    // if the test was run symbolically, output various data about
...
static const char *modelledExternals[] = {
  ...
  "klee_get_obj_size",
  "klee_is_symbolic",
  "klee_is_symbolic_in_symbolic_mode",
  "klee_make_concrete",
  "klee_make_symbolic",
  ...
  "klee_print_range",
  "klee_report_error",
  "klee_require_in_list",
  "klee_obtain_true_index",
  "klee_set_forking",
  "klee_silent_exit",
  "klee_warning",
  "klee_warning_once",
  ...
  "kontest_internal_mode",
  "kontest_internal_begin_block",
  "kontest_internal_end_block",
  "kontest_internal_loop",
...
static llvm::Module *linkWithUclibc(llvm::Module *mainModule, StringRef libDir) {
  ...
  klee_note("Using klee-uclibc : %s", uclibcBCA.c_str());
  return mainModule;
}
#endif
...
int main(int argc, char **argv, char **envp) {
  ...
  if (WriteCov && !OutputIStats) {
    klee_warning("--write-cov requires --output-istats; enabling");
    OutputIStats = true;
  }
  ...
  if (WithPOSIXRuntime) {
    ...
    klee_note("Using model: %s", Path.c_str());
    mainModule = klee::linkWithLibrary(mainModule, Path.c_str());
    assert(mainModule && "unable to link with simple model");
  }

  if (WithKontestRuntime) {
    // Link kontest library
    SmallString<128> Path(Opts.LibraryDir);
    llvm::sys::path::append(Path, "kontest.bc");
    mainModule = klee::linkWithLibrary(mainModule, Path.c_str());
    assert(mainModule && "unable to link with kontest library");
  }
  ...
  uint64_t skeletons =
      *theStatisticManager->getStatisticByName("Skeletons");
  uint64_t pathsExplored =
      *theStatisticManager->getStatisticByName("ConcreteTests");
  uint64_t pathsExpanded =
      *theStatisticManager->getStatisticByName("SymbolicTests");
  uint64_t pathsAbandoned =
      *theStatisticManager->getStatisticByName("AbandonedTests");

  assert(pathsExplored >= pathsAbandoned && "more paths abandoned than executed in the first place!");
  unsigned pathsCompleted = pathsExplored - pathsAbandoned;
  float ratio = ((float) pathsExpanded) / pathsExplored;
  std::snprintf(buf, 256, "%g", ratio);

  handler->getInfoStream()
    << "KLEE: done: explored scenario skeletons = " << skeletons << "\n"
    << "KLEE: done: explored paths = " << pathsExplored << "\n"
    << "KLEE: done: expanded paths = " << pathsExpanded
    << " (ratio " << buf << ")\n";

  float abandonRatio = ((float) pathsAbandoned) / pathsExplored;
  std::snprintf(buf, 256, "%g", abandonRatio);
  handler->getInfoStream()
    << "KLEE: done: abandoned paths = " << pathsAbandoned
    << " (ratio " << buf << ")\n";

  uint64_t divergences = *theStatisticManager->getStatisticByName("Divergences");
  float divRatio = ((float) divergences) / pathsExplored;
  std::snprintf(buf, 256, "%g", divRatio);
  handler->getInfoStream()
    << "KLEE: done: diverged paths = " << divergences
    << " (ratio " << buf << ")\n";
  ...
  stats << "KLEE: done: total instructions = "
        << instructions << "\n";
  if (skeletons > 0)
    stats << "KLEE: done: scenario skeletons = " << skeletons << "\n";
