...
def testFile(name, klee_path, lli_path):
    ...
    klee_cmd = klee_path.split() + ['--output-dir=' + klee_out_path,  '--no-output', '--optimize=false', exeFile]
    ...
    # KLEE output should equal lli output, since we created no symbolics
    # and so will only do a concrete run
    if lliOut != kleeOut:
