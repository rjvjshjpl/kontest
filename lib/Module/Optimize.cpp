...
void Optimize(Module *M, const std::string &EntryPoint) {
  ...
  if (!DisableOptimizations) {
    ...
    // internal.
    //
    // We do not mark memcpy, memmove, or memset internal since calls to them
    // may be introduced later by IntrinsicLowering, and this would cause
    // disaster if they were optimized out or their signatures were changed.
    if (!DisableInternalize) {
      ModulePass *pass = createInternalizePass(
          std::vector<const char *> { EntryPoint.c_str(),
