...
int close(int fd) {
  ...
  // If the file is not symbolic, actually close it, unless it's
  // stdin, stdout, or stderr (since KLEE itself is using those).
  // The fact that these three files are not reset when starting a new test
  // could lead to divergences in certain programs, but that seems unavoidable.
  // Such programs should be tested with --sym-stdin and --sym-stdout anyway.
  if (!f->dfile && f->fd > 2) {
    /* if a concrete fd */
...
static void *__concretize_ptr(const void *p) {
  ...
  return (void *) klee_make_concrete((long) p);
}

static size_t __concretize_size(size_t s) {
  return klee_make_concrete((long) s);
}
