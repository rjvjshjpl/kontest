...
namespace std
{
  // hash function for refs: just hashes the referenced object
  template<typename T>
  struct hash< klee::ref<T> >
  {
    std::size_t operator()(const klee::ref<T> &s) const {
      assert(!s.isNull() && "tried to hash null ref!");
