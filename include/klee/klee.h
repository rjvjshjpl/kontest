...
extern "C" {
  ...
  void klee_define_external_object(void *addr, size_t nbytes);
  ...
  void klee_make_symbolic_range(void *addr, size_t nbytes, const char *name, int lowerBound, int upperBound);
  ...
  // Horrendous macro pretending to be a function, to allow some convenient
  // variadic notation inside C; use with care!
  #define klee_pick(var, ...) do {                                                        \
    int __klee_pick_vals[] = { __VA_ARGS__ };                                             \
    var = klee_choice(#var, sizeof(__klee_pick_vals)/sizeof(int), __klee_pick_vals);      \
  } while (0)
...
#undef KLEE_GET_VALUE_PROTO
  ...
  long klee_make_concrete(long val);
