...
namespace klee {
  ...
  ref<klee::ConstantExpr> Executor::evalConstant(const Constant *c) {
    if (const llvm::ConstantExpr *ce = dyn_cast<llvm::ConstantExpr>(c)) {
      ...
      if (const ConstantInt *ci = dyn_cast<ConstantInt>(c)) {
        ...
        return memory->createNullPointer();
        ...
        for (unsigned i = cds->getNumElements(); i != 0; --i) {
          ref<Expr> kid = evalConstant(cds->getElementAsConstant(i - 1));
          kids.push_back(kid);
        }
  ...
  ref<ConstantExpr> Executor::evalConstantExpr(const llvm::ConstantExpr *ce) {
    ...
    default :
      ce->dump();
      klee_error("unknown ConstantExpr type with opcode: %d", ce->getOpcode());
    ...
    case Instruction::ICmp: {
      ...
      default: llvm_unreachable("unhandled ICmp predicate");
    ...
    case Instruction::FCmp:
      klee_error("floating point ConstantExprs unsupported");
    }
