...
namespace klee {
  ...
  public:
    ...
    std::string readStringAtAddress(ExecutionState &state, uint64_t address);
...
#define HANDLER(name) void name(ExecutionState &state, \
    ...
    HANDLER(handleDefineExternalObject);
    ...
    HANDLER(handleIsSymbolicInSymbolicMode);
    HANDLER(handleMakeConcrete);
    ...
    HANDLER(handleRequireInList);
    HANDLER(handleObtainTrueIndex);

    HANDLER(handleScenarioMode);
    HANDLER(handleBeginBlock);
    HANDLER(handleEndBlock);
    HANDLER(handleLoopBlock);
#undef HANDLER
  };
} // End klee namespace
