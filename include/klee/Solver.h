...
namespace klee {
  ...
  public:
    ...
    Query withFalse() const {
      return Query(constraints, ConstantExpr::False);
    }
  ...
  public:
    ...
    /// getAllValues - Compute all possible values for the given expression.
    ///
    /// \param [out] results - On success, a vector of values for the expression in
    /// all satisfying assignments.
    ///
    /// \return True on success.
    bool getAllValues(const Query&, std::vector< ref<ConstantExpr> > &results, unsigned maxCount = UINT_MAX);

    /// getAllValues - Compute all possible values for the given expression.
    ///
    /// \param [in] handler - A function that gets passed each found value in turn;
    /// it should return a blocking expression (i.e. one whose truth rules out the
    /// given value).
    ///
    /// \return True on success.
    typedef std::function< ref<Expr>(const ref<Expr> &, const ref<ConstantExpr> &) > GetAllValuesHandler;
    bool getAllValues(const Query&, const GetAllValuesHandler &handler);
